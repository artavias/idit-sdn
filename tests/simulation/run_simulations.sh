		...
		# for nd in "${nd_possibilities[@]}"; do
			# for datarate in "${datarates[@]}"; do
				for iter in `seq $MIN_ITER $MAX_ITER`; do
					...
					cooja_file="$simulation_files_dir"/"ITSDN_n"${nnodes}"_s1_"${topo}"_GRID_z1".csc
					#cooja_file="$simulation_files_dir"/${topo}_${nnodes}_2.csc
					cooja_motes_out_file="$simulation_output_dir"/"ITSDN_n"$nnodes"_top_"$topo"_i"$iter'_z1.txt'
					cooja_log_file="$simulation_output_dir"/"ITSDN_log_n"$nnodes"_top_"$topo"_i"$iter'_z1.txt'
					controller_out_file="$simulation_output_dir"/"controller_ITSDN_n"$nnodes"_top_"$topo"_i"$iter'_z1.txt'
					...
					(
						...
						make TARGET=z1 clean -f Makefile_enabled_node
						make TARGET=z1 -f Makefile_enabled_node sink-node

						make TARGET=z1 clean -f Makefile_enabled_node
						make TARGET=z1 -f Makefile_enabled_node enabled-node

						make TARGET=z1 clean -f Makefile_controller_node
						make TARGET=z1 -f Makefile_controller_node

						make TARGET=z1 clean -f Makefile_enabled_node
						make TARGET=z1 -f Makefile_enabled_node management-sink
						
						make TARGET=z1 clean -f Makefile_enabled_node
						make TARGET=z1 -f Makefile_enabled_node attack-one
